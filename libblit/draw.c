...
void draw(Bitmap *d, Point p, Bitmap *s, Rectangle r, int v,Fcode f){
	...
	if(bfunc == UseCopyArea)
		...
		plane &= ~(plane>>1);
		/*
		 * CopyPlane can only do func code S, so copy src rect
		 * into a bitmap with the same depth as the dest, then
		 * do  the  bitblt  from  the tmp.  This won't recurse
		 * again because we only get UseCopyPlane with differ-
		 * ing bitmap depths
		 */
