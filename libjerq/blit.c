...
void initdisplay(int argc, char *argv[]){
	...
	ulong planes;
	char *stdfont = "DejaVu Sans Mono:style=Bold:size=11";
	char *geom = 0;
	int flags, depth;
...
#ifdef NOTDEF
	if( (fp = XftFontOpenName(_dpy, scr, stdfont)) )
		defont = *fp;
	else
		defont = getfont(scr, "8x13");
	font = &defont;
#endif
	if( !(font = XftFontOpenName(_dpy, scr, stdfont)) )
		font = XftFontOpenName(_dpy, scr, "8x13");
	...
	if( !(sizehints.flags & USSize) ){
		sizehints.width = defont.max_advance_width * 80;
		sizehints.height = (defont.ascent +
				 defont.descent) * 24;
	...
	xswa.colormap = DefaultColormap(_dpy, scr);
	...
	screen.draw = XftDrawCreate(_dpy, (Pixmap)screen.id, _vis, _cmap);
...
Font getfont(int screen, char *s){
	Font *fp;

	fp = XftFontOpenName(_dpy, screen, s);
	return fp ? *fp : defont;
}
...
#ifdef FREETYPE2
XRenderColor makecol(ulong rgb){
	XRenderColor color;
