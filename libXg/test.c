...
void main(int argc, char **argv){
	...
	p1 = addpt(screen.r.min, Pt(15,15));
	p2 = subpt(screen.r.max, Pt(15,15));
	p3 = divpt(addpt(p1,p2),2);
	...
	if(screen.ldepth > 1 && screen.ldepth < 4){
		cont("color");
		colorinit();
		p3 = p1;
		rx *= 2;
		ry *= 2;
		for(i = 0; i<Ncol; i++) {
			texture(&screen, Rpt(p3,addpt(p3,Pt(rx,ry/Ncol))),
				rgbbitmap[i], S);
        		string(&screen, addpt(p3,Pt(15,15)), font, colors[i], DxorS);
			p3.y += ry/Ncol;
		}
		printcolmap();
		cont("invert colmap");
	...
	border(&screen, Rpt(p1, addpt(p1,Pt(16,16))), -2, F);
...
cont(char *msg)
{
	...
	mp = addpt(screen.r.min, Pt(20,20));
