...
void init(){
	diagrect=Drect;
	diagrect.min.y=diagrect.max.y-(defont.height+2*M);
	diagclr=F_CLR;
	diag=txalloc(diagrect);
	txinit(diag);
	/* a little adjustment ... */
	diag->frame->rect.min.x-=3*SCROLLWIDTH/2;
	diag->frame->scrollrect.max.x=diag->frame->scrollrect.min.x;
	curse(diag->frame);
	screenrect=Drect;
	screenrect.max.y=diagrect.min.y;
	current=0;
	worktext=0;
}
/*ARGSUSED*/
	...
	Point pt;
{
	...
	ontop(t);
	r=f->scrollrect;
	n=muldiv(y-r.min.y, YMAX, r.max.y-r.min.y);
	if(n<0)
		n=0;
	if(n>YMAX)
		n=YMAX;
	...
	Text *t;
{
	...
	f->scrolly=y;
	r=f->scrollrect;
	y=muldiv(y, r.max.y-r.min.y-2*M, YMAX)+r.min.y+M;
	Rectf(r, F_OR);
	r.min.y=y;
	r.max.y=y+2;
	r.min.x++;
	...
	Point pt;
{
	...
	selectf(f, F_XOR);
	r=f->scrollrect;
	n=muldiv(pt.y-r.min.y-M, f->nlines, r.max.y-r.min.y-2*M);
	if(n<0)
		n=0;
	if(but==B1)	/* backwards */
		n= -n;
