...
static void *stdmalloc(ulong);
static void stdfree(union store*);
static int draincache(void);
#ifdef debug
static int allock(union store*);
static int cached(union store*);
#endif
...
static void *stdmalloc(ulong nw){
...
void ialloc(char *qq, unsigned nbytes){
...
#ifdef debug
static int cached(union store *p){
